...
class ViolationClipRecorder:
    ...
    def _process_clip_worker(
        ...
        try:
            ...
            # 3. Draw annotations. The box is only drawn near the trigger instant;
            #    every frame gets a T±offset tag so the reviewer can tell evidence
            #    from context.
            label = "violation"
            if detection:
            ...
            for ts, frame in buffered_frames:
            ...
            # 4. Encode to browser-compatible H.264 MP4 using FFmpeg
            ...
            safe_tenant = _sanitise_key_segment(tenant_id)
            safe_camera = _sanitise_key_segment(camera_id)
            s3_key = f"violations/{safe_tenant}/{safe_camera}/{date_str}/{violation_id}.mp4"
            ...
            put_kwargs = {
                ...
                # Audit P2: surveillance footage of identifiable people is encrypted
                # at rest, and tagged so an S3 lifecycle rule can expire clips
                # independently of the still frames.
            ...
            # 6. Persist the URL on the violation record, with retries.
